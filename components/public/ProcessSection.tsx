      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="mt-12 grid md:grid-cols-4 gap-8">
          {steps.map((step, idx) => (
            <div
              key={idx}
              className="relative bg-white rounded-lg shadow-md p-6 text-center hover:shadow-xl transition duration-300 group"
            >
              {/* Icon */}
              ...
              {idx < steps.length - 1 && (
                <div className="hidden md:block absolute top-1/2 right-0 w-8 h-0.5 bg-indigo-200"></div>
              )}
            </div>
          ))}
        </div>
