...
import { deleteSubCategoryAction } from "@/modules/subcategory/subcategory.action";

interface Props {
  subCategories: SubCategoryDTO[];
}

export default function SubCategoriesTable({ subCategories }: Props) {
      ...
      <div className="flex flex-col gap-1">
       ...
       <div className="flex justify-between items-center">
         <div>
            ...
            Manage all blog Sub-Categories from here
            </p>
         </div>
         <CreateButton label="New" onClick={()=>router.replace('/admin/blogs/subcategories/create')} className="text-white py-4 px-4" />
       </div>
      </div>
              ...
              {subCategories.map((subCat, index) => (
                ...
                >
                  <td className="px-6 py-4 font-medium text-gray-900">
                    {typeof subCat.category==="string" ?"-":subCat.category.name}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-3">
                      <button
                        ...
                        onClick={() =>
                          router.push(`/admin/blogs/subcategories/update/${subCat._id}`)
                        }
                      >
                        <FiEdit2 size={16} />
                      </button>
                      ...
                      <ConfirmActionDialog
                        ...
                        confirmText="Delete"
                        danger
                        action={() => deleteSubCategoryAction(subCat._id)}
                        trigger={
                            <button
                            title="Delete"
